...
async function main() {
    ...
    const client = new issuer.Client({
        ...
        response_types: ["code"],
        scope: "openid profile universe-messaging-service:publish universe:write universe.place:write user.inventory-item:read asset:write asset:read",
    ...
    app.post("/get_inv", checkLoggedIn, async (req, res) => { //netusim co to e
       
    });

    app.post("/get_games", checkLoggedIn, async (req, res) => { //netusim co to e v2
        console.log(req.signedCookies.tokenSet.access_token.claims())
        const apiUrl = `https://apis.roblox.com/v2/users/${req.signedCookies.tokenSet.access_token.claims()}/games`;
